...
parser.add_argument('--logs_save_dir', default='../exp_logs', type=str,
...
rtpt = RTPT(name_initials='MK', experiment_name=f'TFDC-{args.training_mode}-{args.pretrain_dataset}{f"2{args.target_dataset}"if args.training_mode == "fine_tune_test" else ""}', max_iterations=configs.num_epoch)
# Load Model
...
Trainer(TFC_model, model_optimizer, classifier, classifier_optimizer, train_dl, valid_dl, test_dl, device,
        logger, configs, experiment_log_dir, training_mode,rtpt)

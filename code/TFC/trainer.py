...
def model_finetune(
    ...
    for data, labels, aug1, data_f, aug1_f in val_dl:
        ...
        prc_bs = average_precision_score(
            onehot_label.detach().cpu().numpy(), pred_numpy
        )
    ...
    print(
        " Finetune: loss = %.4f| Acc=%.4f | Precision = %.4f | Recall = %.4f | F1 = %.4f| AUROC=%.4f | AUPRC = %.4f"
        % (
            ave_loss,
            ave_acc * 100,
            precision * 100,
...
def model_test(
    ...
    print(
        "MLP Testing: Acc=%.4f| Precision = %.4f | Recall = %.4f | F1 = %.4f | AUROC= %.4f | AUPRC=%.4f"
        % (
            acc * 100,
            precision * 100,
            recall * 100,
